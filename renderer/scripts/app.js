...
[btnDraw, btnRectangle, btnCircle, btnLine, btnText, btnPan, btnEraserPaint, btnEraserStroke].forEach((btn, i) => {
    ...
    btn.addEventListener('click', () => { window.currentTool = tools[i]; updateActiveToolButton(); });
});
...
function showToolbarTemporarily() {
    ...
    hideTimeout = setTimeout(() => { if (!window.isDrawingMode) { toolbar.style.opacity = '0'; toolbar.style.transform = 'translateY(-8px)'; } }, 2500);
}
